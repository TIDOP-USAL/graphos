...
DenseTaskBase::DenseTaskBase(const ImageRepository &imageRepo,
...
DenseTaskBase::~DenseTaskBase() = default;

void DenseTaskBase::enableCuda(bool enable)
{
    mCuda = enable;
}

auto DenseTaskBase::isCudaEnabled() const -> bool
{
    return mCuda;
}

auto DenseTaskBase::denseModel() const -> tl::Path
{
    return mDenseModel;
}

auto DenseTaskBase::report() const -> DensificationReport
{
    return mReport;
}

void DenseTaskBase::setUndistortImagesFormat(UndistortImages::Format format)
{
    mFormat = format;
}

auto DenseTaskBase::undistort(const QString &dir) const -> void
{

    try {

        UndistortImages undistort(mImageRepo,
                                  mCameraRepo,
                                  dir.toStdString(),
...
auto DenseTaskBase::outputPath() const -> tl::Path
{
    return mOutputPath;
}

auto DenseTaskBase::images() const -> const ImageRepository &
{
    return mImageRepo;
}

auto DenseTaskBase::cameras() const -> const CameraRepository &
{
    return mCameraRepo;
}

auto DenseTaskBase::poses() const -> const CameraPosesRepository &
{
    return mPosesRepo;
}

auto DenseTaskBase::groundPoints() const -> const std::vector<GroundPoint>&
{
    return mGroundPoints;
}

void DenseTaskBase::setDenseModel(const tl::Path &denseModel)
{
    mDenseModel = denseModel;
}

void DenseTaskBase::autoSegmentation() const
{
#ifdef GRAPHOS_GUI
    try {
