...
//  }
//#else
//  std::shared_ptr<graphos::FeatureExtractor> feat_extractor = std::make_shared<SiftCudaDetectorDescriptor>(mMaxFeaturesNumber,
//                                                                                                  mOctaveResolution,
//                                                                                                  mEdgeThreshold,
//                                                                                                  mSigma,
//                                                                                                  mContrastThreshold);
//#endif
//
//  colmap::Camera prev_camera_;
//  prev_camera_.SetCameraId(kInvalidCameraId);
//  prev_camera_.SetModelIdFromName(reader_options.camera_model);
...
//    //
//    //image_data.image.SetName(image_path);
//    //
//    //const bool exists_image = database.ExistsImageWithName(image_data.image.Name());
//    //
//    //if (exists_image) {
//    //  image_data.image = database.ReadImageWithName(image_data.image.Name());
//    //  // Siempre estoy borrando la base de datos
//    //  //const bool exists_keypoints = database.ExistsKeypoints(image_data.image.ImageId());
//    //  //const bool exists_descriptors = database.ExistsDescriptors(image_data.image.ImageId());
//    //
...
//    //  return 1;// colmap::ImageReader::Status::BITMAP_ERROR;
//    //}
//    //
//    //if (exists_image) {
//    //  const colmap::Camera current_camera = database.ReadCamera(image_data.image.CameraId());
//    //
//    //  if (static_cast<size_t>(image_data.bitmap.Width()) != current_camera.Width() ||
//    //      static_cast<size_t>(image_data.bitmap.Height()) != current_camera.Height()) {
//    //    return 1;// Status::CAMERA_EXIST_DIM_ERROR;
//    //  }
//    //
...
//    //      if (image_data.bitmap.ExifFocalLength(&focal_length)) {
//    //        prev_camera_.SetPriorFocalLength(true);
//    //      } else {
//    //        focal_length = reader_options.default_focal_length_factor *
...
//    //    //}
//    //
//    //    prev_camera_.SetWidth(static_cast<size_t>(image_data.bitmap.Width()));
//    //    prev_camera_.SetHeight(static_cast<size_t>(image_data.bitmap.Height()));
//    //
//    //    if (!prev_camera_.VerifyParams()) {
//    //      return 1;// Status::CAMERA_PARAM_ERROR;
//    //    }
//    //
//    //    prev_camera_.SetCameraId(database.WriteCamera(prev_camera_));
//    //  }
//    //
//    //  image_data.image.SetCameraId(prev_camera_.CameraId());
//    //
//    //  //////////////////////////////////////////////////////////////////////////////
//    //  // Extract GPS data.
//    //  //////////////////////////////////////////////////////////////////////////////
//    //
//    //  if (!image_data.bitmap.ExifLatitude(&image_data.image.TvecPrior(0)) ||
//    //      !image_data.bitmap.ExifLongitude(&image_data.image.TvecPrior(1)) ||
//    //      !image_data.bitmap.ExifAltitude(&image_data.image.TvecPrior(2))) {
//    //    image_data.image.TvecPrior().setConstant(std::numeric_limits<double>::quiet_NaN());
//    //  }
//    //}
//
//    colmap::image_t image_id;
...
//    
//      //featextract_mutex.lock();
//      bool exist_image = database.ExistsImageWithName(image_path);
//      //featextract_mutex.unlock();
//      if (!exist_image) {
//
//        //colmap::Camera camera_colmap;
...
//
//        //featextract_mutex.lock();
//        bool exists_camera = database.ExistsCamera(camera_id);
//        //featextract_mutex.unlock();
//
//        if (!exists_camera) {
//
...
//          size_t height = static_cast<size_t>(camera.height());
//          //if (round(1.2 * std::max(width, height)) == round(focal_length)) {
//          //  image_data.camera.SetPriorFocalLength(false);
//          //} else {
//          //  image_data.camera.SetPriorFocalLength(true);
//          //}
//
//          double focal_length = 0.0;
//          if (image_data.bitmap.Read(image_path, false)) {
...
//              image_data.camera.SetPriorFocalLength(false);
//            }
//          } /// Ver si da error que hago ...
//
//
//
//          image_data.camera.InitializeWithId(camera_model_id, focal_length, width, height);
//          camera_id = database.WriteCamera(image_data.camera);
//          image_data.camera.SetCameraId(camera_id);
//        }
//
//        //colmap::Image image_colmap;
//        image_data.image.SetName(image_path);
//        image_data.image.TvecPrior(0) = image.cameraPose().position().x;
//        image_data.image.TvecPrior(1) = image.cameraPose().position().y;
//        image_data.image.TvecPrior(2) = image.cameraPose().position().z;
//        image_data.image.QvecPrior(0) = image.cameraPose().quaternion().w;
//        image_data.image.QvecPrior(1) = image.cameraPose().quaternion().x;
//        image_data.image.QvecPrior(2) = image.cameraPose().quaternion().y;
//        image_data.image.QvecPrior(3) = image.cameraPose().quaternion().z;
//        image_data.image.SetCameraId(camera_id);
//
//        //featextract_mutex.lock();
//        image_id = database.WriteImage(image_data.image, false);
//        image_data.image.SetImageId(image_id);
//        //featextract_mutex.unlock();
//
//      } else {
//
...
//      /* Lectura de imagen */
//      
//      image_data.bitmap.Read(image.path().toStdString(), false);
//      image_data.status = colmap::ImageReader::Status::SUCCESS;
//    }
//
//#endif
//
...
//    }
//#else
//        cv::Size size(image_data.bitmap.Width(), image_data.bitmap.Height());
//        double max_dimension = std::max(size.width, size.height);
//        if (mMaxImageSize > 0 && mMaxImageSize < max_dimension) {
//          scale = max_dimension / mMaxImageSize;
//          size.width /= scale;
//          size.height /= scale;
//          image_data.bitmap.Rescale(size.width, size.height);
//        }
//#endif
//
//
...
//      /// Graphos
//      try {
//        feat_extractor->run(image_data.bitmap, image_data.keypoints, image_data.descriptors);
//        success = true;
//      } catch (...) {
//        success = false;
//      }
...
//#else
//      if (scale > 1) {
//        for (auto& featureKeypoint : image_data.keypoints) {
...
//      // Escritura en la base de datos
//      //image_data.image.SetImageId(database.WriteImage(image_data.image));
//      database.WriteKeypoints(image_data.image.ImageId(), image_data.keypoints);
//      database.WriteDescriptors(image_data.image.ImageId(), image_data.descriptors);
//      project.addFeatures(image.name(), image.name() + "@" + project.database());
//#endif
//    }
//
//
