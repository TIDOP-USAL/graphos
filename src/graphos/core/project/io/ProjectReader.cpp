...
void readFeatureConfig(QXmlStreamReader &stream, Project &project)
{
    ...
    std::string algorithm = attrs.value("algorithm").toString().toStdString();

    auto config = FeatureFactory::create(algorithm);
    if (!config) {
        stream.skipCurrentElement();
        return;
    }
...
void readFeatureRepository(QXmlStreamReader &stream, Project &project)
{
...
void ProjectReader::readFeatures(QXmlStreamReader &stream, Project &project)
{
    while (stream.readNextStartElement()) {
        ...
        } else if (stream.name() == "Repository") {
            readFeatureRepository(stream, project);
        } else {
            stream.skipCurrentElement();
        }
    }
...
void ProjectReader::readMatches(QXmlStreamReader &stream, Project &project)
{
    while (stream.readNextStartElement()) {
        if (stream.name() == "Config") {
            ...
            auto config = std::make_shared<MatchingProperties>();
            ...
            project.setMatchingConfig(config);
            ...
            project.setMatchingReport(report);
...
void ProjectReader::readOrientation(QXmlStreamReader &stream, Project &project)
{
    while (stream.readNextStartElement()) {
        if (stream.name() == "Config") {
            ...
            project.setCameraPosesFile(streamToStdString(stream));
        } else if (stream.name() == "Report") {
...
void ProjectReader::readDensification(QXmlStreamReader &stream, Project &project)
{
    while (stream.readNextStartElement()) {
        if (stream.name() == "Config") {
            ...
            std::string method("mvs");
            if (stream.attributes().hasAttribute("method"))
                method = stream.attributes().value("method").toString().toStdString();

            auto config = DensificationPropertiesFactory::create(method);
            ...
            project.setDensificationConfig(config);

        } else if (stream.name() == "DenseModel") {
            project.setDenseModel(streamToStdString(stream));
        } else if (stream.name() == "Report") {

            DensificationReport report;

            while (stream.readNextStartElement()) {
                if (stream.name() == "Points") {
                    report.points = streamToInt(stream);
                } else if (stream.name() == "Method") {
                    report.method = streamToStdString(stream);
                } else if (stream.name() == "Time") {
                    report.time = streamToDouble(stream);
                } else if (stream.name() == "Cuda") {
                    report.cuda = streamToBoolean(stream);
                } else
                    stream.skipCurrentElement();
            }

            project.setDensificationReport(report);
