...
ImportOrientationTask::ImportOrientationTask(const std::vector<Image> &images,
                                             const std::map<int, Camera> &cameras,
                                             const QString &outputPath,
...
ImportOrientationTask::~ImportOrientationTask()
{
}

std::map<int, Camera> ImportOrientationTask::cameras() const
{
  return mCameras;
}

void ImportOrientationTask::execute(tl::Progress *progressBar)
{
  try {
    ...
    std::unordered_map<size_t, colmap::image_t> graphos_to_colmap_id;
    {
      ...
      for (const auto &image : mImages) {
        ...
        for (const auto &colmap_image : database.ReadAllImages()) {
          ...
          if (image_path.equivalent(colmap_image_path)) {
            graphos_to_colmap_id[image.id()] = colmap_image.ImageId();
            break;
          }
        }
    ...
    /// 1 - Escritura images.txt
    {
      ...
      for (const auto &image : mImages) {
        ...
        ofs << std::fixed << graphos_to_colmap_id[image.id()] << " " << QString::number(quaternion.w, 'g', 10).toStdString() << " "
    ...
    {
      tl::Path reconstruction_path(mOutputPath.toStdWString());
      ...
      auto gp_writer = GroundPointsWriterFactory::create("GRAPHOS");
      gp_writer->setGroundPoints(ground_points);
      tl::Path ground_points_path(mOutputPath.toStdString());
      ground_points_path.append("ground_points.bin");
      gp_writer->write(ground_points_path);
      ...
      auto poses_writer = CameraPosesWriterFactory::create("GRAPHOS");
      poses_writer->setCameraPoses(camera_poses);
      tl::Path poses_path(mOutputPath.toStdString());
      poses_path.append("poses.bin");
      poses_writer->write(poses_path);
      ...
      tl::Path sparse_path(mOutputPath.toStdString());
      sparse_path.append("sparse.ply");
      OrientationExport orientationExport(&reconstruction);
      //orientationExport.exportBinary(QString::fromStdString(sparse_path.toString()));
      orientationExport.exportPLY(QString::fromStdString(sparse_path.toString()));
      ...
      /// writeOffset
      tl::Path offset_path(mOutputPath.toStdString());
      offset_path.append("offset.txt");
      std::ofstream stream(offset_path.toString(), std::ios::trunc);
      if (stream.is_open()) {
